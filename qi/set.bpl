...
//! for found in N n v found; do for eq in '==' '!='; do for c in -1 0; do for N in N n v found; do echo "invariant b$((++idx)) ==> (forall i:int, j:int :: $found $eq $c ==> 0 <= i && i < $N && 0 <= j && j < $N && i != j ==> s[i] != s[j]);"; done; done; done; done
