...
implementation reverse_sorted(P#l: $ptr) returns ($result: $ptr)
{
  ...
  anon4:
    ...
    assume true;
    while (true)
      invariant F#srtl($s, $phys_ptr_cast(local.l, ^s_node));
      invariant F#rsrtl($s, $phys_ptr_cast(L#r, ^s_node));
      invariant $oset_disjoint(F#srtl_reach($s, $phys_ptr_cast(local.l, ^s_node)), F#rsrtl_reach($s, $phys_ptr_cast(L#r, ^s_node)));
      invariant $non_null($phys_ptr_cast(L#r, ^s_node)) && $non_null($phys_ptr_cast(local.l, ^s_node)) ==> F#sll_max_key($s, $phys_ptr_cast(L#r, ^s_node)) <= $rd_inv($s, s_node.key, $phys_ptr_cast(local.l, ^s_node));
      invariant F#sll_keys(loopState#0, $phys_ptr_cast(P#l, ^s_node)) == $intset_union(F#sll_keys($s, $phys_ptr_cast(local.l, ^s_node)), F#sll_keys($s, $phys_ptr_cast(L#r, ^s_node)));
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Csorted?5Creverse_sorted.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Csorted?5Creverse_sorted.c);
