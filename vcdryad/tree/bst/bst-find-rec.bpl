...
procedure bst_find_rec(P#x: $ptr, P#k: int) returns ($result: int);
  ...
  ensures F#bst_max_key($s, $phys_ptr_cast(P#x, ^b_node)) == F#bst_max_key(old($s), $phys_ptr_cast(P#x, ^b_node));
  ensures F#bst_min_key($s, $phys_ptr_cast(P#x, ^b_node)) == F#bst_min_key(old($s), $phys_ptr_cast(P#x, ^b_node));
          ...
          anon5:
            ...
            else
            {
              anon4:
                ...
                assume $full_stop_ext(#tok$2^31.17, $s);
...
const unique #tok$2^31.17: $token;
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Ctree?5Cbst?5Cbst?2Dfind?2Drec.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Ctree?5Cbst?5Cbst?2Dfind?2Drec.c);

const unique #file^z?3A?5Cinvariantsynthesis?5Csubmit?5Ctree?5Cbst?5Cdryad_bst.h: $token;

axiom $file_name_is(1, #file^z?3A?5Cinvariantsynthesis?5Csubmit?5Ctree?5Cbst?5Cdryad_bst.h);
