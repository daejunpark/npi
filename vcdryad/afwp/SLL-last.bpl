...
const unique ^$#SLL_last.c..36263#3: $ctype;

axiom $def_fnptr_type(^$#SLL_last.c..36263#3);

type $#SLL_last.c..36263#3;
...
implementation SLL_last(P#h: $ptr) returns ($result: $ptr)
{
  ...
  anon4:
    ...
    while (true)
      ...
      invariant F#sll_lseg($s, $phys_ptr_cast(P#h, ^s_node), $phys_ptr_cast(L#j, ^s_node));
      invariant $oset_disjoint(F#sll_lseg_reach($s, $phys_ptr_cast(P#h, ^s_node), $phys_ptr_cast(L#j, ^s_node)), F#sll_reach($s, $phys_ptr_cast(L#j, ^s_node)));
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cafwp?5CSLL?2Dlast.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cafwp?5CSLL?2Dlast.c);
