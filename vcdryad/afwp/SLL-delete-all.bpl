...
const unique ^$#SLL_delete_all.c..36263#3: $ctype;

axiom $def_fnptr_type(^$#SLL_delete_all.c..36263#3);

type $#SLL_delete_all.c..36263#3;
      ...
      anon3:
        ...
        if ($non_null($phys_ptr_cast(L#i, ^s_node)))
        {
          anon1:
            ...
            assert $top_writable($s, #wrTime$2^3.3, $phys_ptr_cast(L#j, ^s_node));
            // assert @writes_check(@_vcc_extent(@state, j)); 
            assert (forall #writes$2^24.5: $ptr :: { $dont_instantiate(#writes$2^24.5) } $set_in(#writes$2^24.5, $extent($s, $phys_ptr_cast(L#j, ^s_node))) ==> $top_writable($s, #wrTime$2^3.3, #writes$2^24.5));
            // stmt _vcc_free(j); 
            call $free($phys_ptr_cast(L#j, ^s_node));
            assume $full_stop_ext(#tok$2^24.5, $s);
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cafwp?5CSLL?2Ddelete?2Dall.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cafwp?5CSLL?2Ddelete?2Dall.c);
