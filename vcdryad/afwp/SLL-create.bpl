...
const unique ^$#SLL_create.c..36263#3: $ctype;

axiom $def_fnptr_type(^$#SLL_create.c..36263#3);

type $#SLL_create.c..36263#3;
...
implementation create(P#sz: int) returns ($result: $ptr)
{
  ...
  anon4:
    ...
    assume true;
    while (true)
      invariant F#sll($s, $phys_ptr_cast(L#h, ^s_node));
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cafwp?5CSLL?2Dcreate.c: $token;

axiom $file_name_is(3, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cafwp?5CSLL?2Dcreate.c);

const unique #file^z?3A?5Cinvariantsynthesis?5Csubmit?5Cafwp?5Cdryad_SLL.h: $token;

axiom $file_name_is(2, #file^z?3A?5Cinvariantsynthesis?5Csubmit?5Cafwp?5Cdryad_SLL.h);
