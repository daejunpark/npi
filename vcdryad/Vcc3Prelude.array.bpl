...
// Intuitively:
//   $index_within(p, arr) = ($addr(p) - $addr(arr)) div $sizeof($typ(arr))
...
function {:inline false} $rd_phys_ptr(s:$state, f:$field, p:$ptr, t:$ctype) : $ptr
...
function $int_min(x:int, y:int) : int
  { if x < y then x else y }

function $int_max(x:int, y:int) : int
  { if x > y then x else y }

// INTSET

type $intset = [int]bool;
const unique ^$#intset : $ctype;

function {:builtin "MapConst"} imapconstbool(bool) : $intset;
function {:builtin "MapAnd"} imapand($intset, $intset) : $intset;
function {:builtin "MapOr"} imapor($intset, $intset) : $intset;
function {:builtin "MapNot"} imapnot($intset) : $intset;
// function {:builtin "MapIte"} imapitebool($intset, $intset, $intset) : $intset;
// function {:builtin "MapIff"} imapiff($intset, $intset) : $intset;
// function {:builtin "MapImp"} imapimp($intset, $intset) : $intset;
// function {:builtin "MapConst"} imapconstint(int) : [int]int;
// function {:builtin "MapLt"} imaplt([int]int, [int]int): $intset;
// function {:builtin "MapLe"} imaple([int]int, [int]int): $intset;
// function {:builtin "MapIte"} imapiteint($intset, [int]int, [int]int) : [int]int;

function {:inline} $intset_empty() : $intset
  { imapconstbool(false) }

function {:inline} $intset_universe() : $intset
  { imapconstbool(true) }

function {:inline} $intset_singleton(x: int): $intset
  { $intset_empty()[x := true] }

function {:inline} $intset_union(S: $intset, P: $intset): $intset
  { imapor(S, P) }
function {:inline} $intset_union3(S: $intset, P: $intset, R: $intset): $intset
  { imapor(imapor(S, P),R) }
function {:inline} $intset_union_one1(el:int, S:$intset) : $intset
  { S[el := true] }
function {:inline} $intset_union_one2(S:$intset, el:int) : $intset
  { S[el := true] }

function {:inline} $intset_intersect(S:$intset, P:$intset) : $intset
  { imapand(S, P) }

function {:inline} $intset_diff(S:$intset, P:$intset) : $intset
  { imapand(S, imapnot(P)) }

function {:inline} $intset_in(x: int, S: $intset): bool
  { S[x] }

function {:inline} $intset_subset(S: $intset, P: $intset): bool
  { $intset_intersect(S, P) == S }

function {:inline} $intset_disjoint(S: $intset, P: $intset): bool
  { $intset_intersect(S, P) == $intset_empty() }

// function $intset_lt(S:$intset, P:$intset): bool
//   { (forall i:int, j:int :: {P[i], S[j]} ((i <= j) ==> (!P[i] || !S[j]))) }
//   /*{ imaplt(imapiteint(S, imapconstint(1), imapconstint(0)),
//            imapiteint(P, imapconstint(1), imapconstint(1))) == imapconstbool(true) } */
// function $intset_lt_one1(el:int, S:$intset) : bool
//   { (forall i:int :: {S[i]} (i <= el) ==> !S[i]) }
//  // { imaplt(imapconstint(0)[el := 1],
//  //          imapiteint(S, imapconstint(1), imapconstint(0))) == imapconstbool(true) }
// function {:inline} $intset_lt_one2(S:$intset, el:int) : bool
//   { (forall i:int :: {S[i]} (el <= i) ==> !S[i]) }
//   /*{ imaplt(imapiteint(S, imapconstint(1), imapconstint(0)),
//           imapconstint(0)[el := 1]) == imapconstbool(true) } */
// 
// //function $intset_le(S:$intset, P:$intset): bool
// //  { (exists k:int :: (forall i:int :: (S[i] ==> i <= k) && (P[i] ==> k <= i))) }
// //function $intset_le_one1(el:int, S:$intset) : bool
// //  { (forall i:int :: S[i] ==> el <= i) }
// //function $intset_le_one2(S:$intset, el:int) : bool
// //  { (forall i:int :: S[i] ==> i <= el) }
// 
// function $intset_le_one1(el:int, S:$intset) : bool
//   { (forall i:int :: {S[i]} i < el ==> !S[i]) }
// function $intset_le_one2(S:$intset, el:int) : bool
...
type $oset; // = [$ptr]bool;
const unique ^$#oset : $ctype;

function $oset_empty() : $oset;
function $oset_singleton($ptr): $oset;
function $oset_union($oset, $oset): $oset;

function $oset_in($ptr, $oset): bool;
function $oset_disjoint($oset, $oset): bool;

function $oset_intersect($oset, $oset): $oset;
function $oset_diff($oset, $oset) : $oset;

function {:inline} $oset_union_one1(p:$ptr, s:$oset) : $oset
  { $oset_union($oset_singleton(p), s) }
function {:inline} $oset_union_one2(s:$oset, p:$ptr) : $oset
  { $oset_union(s, $oset_singleton(p)) }

function $oset_subset($oset, $oset) : bool;
/*
function {:inline} $oset_subset(s1:$oset, s2:$oset) : bool
  { $oset_diff(s1,s2) == $oset_empty() }
*/
...
*/
axiom (forall #s:$oset :: { $oset_union(#s,#s) } $oset_union(#s,#s) == #s);
//
axiom (forall #s:$oset :: { $oset_union(#s, $oset_empty()) } $oset_union(#s, $oset_empty()) == #s);
axiom (forall #s:$oset :: { $oset_union($oset_empty(), #s) } $oset_union($oset_empty(), #s) == #s);
//
// axiom (forall #s1:$oset, #s2:$oset :: { $oset_union(#s1,#s2) } $oset_union(#s1,#s2) == $oset_union(#s2,#s1));
//
axiom (forall #s1:$oset, #s2:$oset, #s3:$oset :: { $oset_union(#s1,$oset_union(#s2,#s3)) } $oset_union(#s1,$oset_union(#s2,#s3)) == $oset_union($oset_union(#s1,#s2),#s3));

/*
    p \in \empty == false
    p \in \singleton(q) == (p == q)
    p \in (s1 \cup s2) == i \in s1 \or i \in s2
*/
axiom (forall #p:$ptr :: { $oset_in(#p, $oset_empty()) } $oset_in(#p, $oset_empty()) == false);
axiom (forall #p:$ptr, #q:$ptr :: { $oset_in(#p, $oset_singleton(#q)) } $oset_in(#p, $oset_singleton(#q)) == (#p == #q));
axiom (forall #p:$ptr, #s1:$oset, #s2:$oset :: { $oset_in(#p, $oset_union(#s1,#s2)) } $oset_in(#p, $oset_union(#s1,#s2)) == ($oset_in(#p, #s1) || $oset_in(#p, #s2)));

/*
    s \disjoint s == (s == empty)       // NOTE: if s is empty, s is disjoint with itself
    //
    s1 \disjoint s2 == s2 \disjoint s1
    //
    s \disjoint empty == true
    s \disjoint \singleton(p) == p \not\in s
    s \disjoint (s1 \cup s2) == s \disjoint s1 && s \disjoint s2
    //
//  singleton(p) \disjoint singleton(q) == (p != q)     // NOTE: maybe redundant
*/
axiom (forall #s:$oset :: { $oset_disjoint(#s,#s) } $oset_disjoint(#s,#s) == (#s == $oset_empty()));
//
axiom (forall #s1:$oset, #s2:$oset :: { $oset_disjoint(#s1,#s2) } $oset_disjoint(#s1,#s2) == $oset_disjoint(#s2,#s1));
//
axiom (forall #s:$oset :: { $oset_disjoint(#s, $oset_empty()) } $oset_disjoint(#s, $oset_empty()) == true);
axiom (forall #s:$oset :: { $oset_disjoint($oset_empty(), #s) } $oset_disjoint($oset_empty(), #s) == true);
axiom (forall #s:$oset, #p:$ptr :: { $oset_disjoint(#s, $oset_singleton(#p)) } $oset_disjoint(#s, $oset_singleton(#p)) == (!$oset_in(#p,#s))); // NOTE: not mechanically proved yet
axiom (forall #s:$oset, #p:$ptr :: { $oset_disjoint($oset_singleton(#p), #s) } $oset_disjoint($oset_singleton(#p), #s) == (!$oset_in(#p,#s))); // NOTE: not mechanically proved yet
axiom (forall #s:$oset, #s1:$oset, #s2:$oset :: { $oset_disjoint(#s, $oset_union(#s1,#s2)) } $oset_disjoint(#s, $oset_union(#s1,#s2)) == ($oset_disjoint(#s, #s1) && $oset_disjoint(#s, #s2)));
axiom (forall #s:$oset, #s1:$oset, #s2:$oset :: { $oset_disjoint($oset_union(#s1,#s2), #s) } $oset_disjoint($oset_union(#s1,#s2), #s) == ($oset_disjoint(#s1, #s) && $oset_disjoint(#s2, #s)));
//
// axiom (forall #p:$ptr, #q:$ptr :: { $oset_disjoint($oset_singleton(#p),$oset_singleton(#q)) } $oset_disjoint($oset_singleton(#p),$oset_singleton(#q)) == (#p != #q));
...
//     //
//     s \cap empty == empty
//     s \cap (s1 \cup s2) == (s \cap s1) \cup (s \cap s2)
// */
// // axiom (forall #s:$oset :: $oset_intersect(#s,#s) == #s);
// //
// axiom (forall #s1:$oset, #s2:$oset :: $oset_intersect(#s1,#s2) == $oset_intersect(#s2,#s1));
// //
// // axiom (forall #s1:$oset, #s2:$oset, #s3:$oset :: $oset_intersect(#s1,$oset_intersect(#s2,#s3)) == $oset_intersect($oset_intersect(#s1,#s2),#s3));
// //
// axiom (forall #s:$oset :: { $oset_intersect(#s, $oset_empty()) } $oset_intersect(#s, $oset_empty()) == $oset_empty());
// axiom (forall #s:$oset :: { $oset_intersect($oset_empty(), #s) } $oset_intersect($oset_empty(), #s) == $oset_empty());
// axiom (forall #s:$oset, #s1:$oset, #s2:$oset :: { $oset_intersect(#s, $oset_union(#s1,#s2)) } $oset_intersect(#s, $oset_union(#s1,#s2)) == $oset_union($oset_intersect(#s, #s1), $oset_intersect(#s, #s2)));
// axiom (forall #s:$oset, #s1:$oset, #s2:$oset :: { $oset_intersect($oset_union(#s1,#s2), #s) } $oset_intersect($oset_union(#s1,#s2), #s) == $oset_union($oset_intersect(#s1, #s), $oset_intersect(#s2, #s)));
...
/*
    ...
    \empty - s == \empty
    //
//  s - (s1 \cup s2) == (s - s1) \CAP (s - s2)    // NOTE: not cup but CAP
    (s1 \cup s2) - s == (s1 - s) \cup (s2 - s)
*/
axiom (forall #s:$oset :: { $oset_diff(#s,#s) } $oset_diff(#s,#s) == $oset_empty());
//
axiom (forall #s:$oset :: { $oset_diff(#s,$oset_empty()) } $oset_diff(#s,$oset_empty()) == #s);
axiom (forall #s:$oset :: { $oset_diff($oset_empty(),#s) } $oset_diff($oset_empty(),#s) == $oset_empty());
//
// axiom (forall #s:$oset, #s1:$oset, #s2:$oset :: { $oset_diff(#s, $oset_union(#s1,#s2)) } $oset_diff(#s, $oset_union(#s1,#s2)) == $oset_intersect($oset_diff(#s,#s1), $oset_diff(#s,#s2)));
axiom (forall #s:$oset, #s1:$oset, #s2:$oset :: { $oset_diff($oset_union(#s1,#s2), #s) } $oset_diff($oset_union(#s1,#s2), #s) == $oset_union($oset_diff(#s1,#s), $oset_diff(#s2,#s)));
...
/*
    ...
    s <= empty == (s == empty)
    //
    singleton(p) <= s == p in s
    //
    (s1 cup s2) <= s == s1 <= s && s2 <= s
    //
//  singleton(p) <= singleton(q) == (p == q)    // NOTE: maybe redundant
    //
    s1 <= s2 ==> s1 <= (s2 cup s3)
*/
axiom (forall #s:$oset :: { $oset_subset(#s,#s) } $oset_subset(#s,#s) == true);
//
axiom (forall #s:$oset :: { $oset_subset($oset_empty(),#s) } $oset_subset($oset_empty(),#s) == true);
axiom (forall #s:$oset :: { $oset_subset(#s,$oset_empty()) } $oset_subset(#s,$oset_empty()) == (#s == $oset_empty()));
//
axiom (forall #s:$oset, #p:$ptr :: { $oset_subset($oset_singleton(#p),#s) } $oset_subset($oset_singleton(#p),#s) == $oset_in(#p,#s));
//
axiom (forall #s:$oset, #s1:$oset, #s2:$oset :: { $oset_subset($oset_union(#s1,#s2), #s) } $oset_subset($oset_union(#s1,#s2), #s) == ($oset_subset(#s1,#s) && $oset_subset(#s2,#s))); // NOTE: not mechanically proved yet
//
// axiom (forall #p:$ptr, #q:$ptr :: { $oset_subset($oset_singleton(#p),$oset_singleton(#q)) } $oset_subset($oset_singleton(#p),$oset_singleton(#q)) == (#p == #q));
//
axiom (forall #s1:$oset, #s2:$oset, #s3:$oset :: { $oset_subset(#s1, $oset_union(#s2, #s3)) } $oset_subset(#s1,#s2) ==> $oset_subset(#s1, $oset_union(#s2, #s3)));

// /*
//     s1 \disjoint s2 == (s1 \cap s2 == empty)
//     s1 - s2 == s1 - (s1 \cap s2)
// */
// axiom (forall #s1:$oset, #s2:$oset :: { $oset_disjoint(#s1,#s2) } $oset_disjoint(#s1,#s2) == ($oset_intersect(#s1,#s2) == $oset_empty()));
// axiom (forall #s1:$oset, #s2:$oset :: { $oset_diff(#s1,#s2) } $oset_diff(#s1,#s2) == $oset_diff(#s1, $oset_intersect(#s1,#s2)));

/*
    s1 \disjoint s2 ==> s1 - s2 = s1
    s1 \disjoint s2 ==> s2 - s1 = s2
    //
    s1 \disjoint s2 ==> (s1 \cup s2) - s2 == s1
    s1 \disjoint s2 ==> (s1 \cup s2) - s1 == s2
*/
axiom (forall #s1:$oset, #s2:$oset :: { $oset_diff(#s1,#s2) } $oset_disjoint(#s1,#s2) ==> ($oset_diff(#s1,#s2) == #s1));
axiom (forall #s1:$oset, #s2:$oset :: { $oset_diff(#s2,#s1) } $oset_disjoint(#s1,#s2) ==> ($oset_diff(#s2,#s1) == #s2));
//
axiom (forall #s1:$oset, #s2:$oset :: { $oset_diff($oset_union(#s1,#s2), #s1) } $oset_disjoint(#s1,#s2) ==> ($oset_diff($oset_union(#s1,#s2), #s1) == #s2));
axiom (forall #s1:$oset, #s2:$oset :: { $oset_diff($oset_union(#s1,#s2), #s2) } $oset_disjoint(#s1,#s2) ==> ($oset_diff($oset_union(#s1,#s2), #s2) == #s1));

// NOTE: to prove sls_pairwise_sum
/*
    s1 <= s2 ==> (s1 \cup s) <= (s2 \cup s)
    s1 <= s2 ==> s1 <= (s2 cup s3)
    (s1 \cup s) <= s2 ==> s1 <= s2
*/
axiom (forall #s:$oset, #s1:$oset, #s2:$oset :: { $oset_subset($oset_union(#s1,#s), $oset_union(#s2,#s)) } $oset_subset(#s1,#s2) ==> $oset_subset($oset_union(#s1,#s), $oset_union(#s2,#s)));
axiom (forall #s:$oset, #s1:$oset, #s2:$oset :: { $oset_subset(#s1, $oset_union(#s2,#s)) } $oset_subset(#s1,#s2) ==> $oset_subset(#s1, $oset_union(#s2,#s)));
axiom (forall #s:$oset, #s1:$oset, #s2:$oset :: { $oset_subset($oset_union(#s1,#s), #s2) } $oset_subset($oset_union(#s1,#s), #s2) ==> $oset_subset(#s1,#s2));

// NOTE: to prove sl_copy and dl_copy
/*
    s1 <= s2 && !(p in s2) ==> !(p in s1)
*/
axiom (forall #p:$ptr, #s1:$oset, #s2:$oset :: { $oset_subset(#s1,#s2), $oset_in(#p,#s2) } ( $oset_subset(#s1,#s2) && (! $oset_in(#p,#s2)) ) ==> (! $oset_in(#p,#s1)));
