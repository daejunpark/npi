...
const unique ^$#dl_reverse.c..36263#3: $ctype;

axiom $def_fnptr_type(^$#dl_reverse.c..36263#3);

type $#dl_reverse.c..36263#3;
...
implementation dl_reverse(P#lst: $ptr) returns ($result: $ptr)
{
  ...
  anon7:
    ...
    assume true;
    while (true)
      invariant F#dll($s, $phys_ptr_cast(L#rev, ^d_node));
      invariant F#dll($s, $phys_ptr_cast(L#curr, ^d_node));
      invariant $oset_disjoint(F#dll_reach($s, $phys_ptr_cast(L#rev, ^d_node)), F#dll_reach($s, $phys_ptr_cast(L#curr, ^d_node)));
      ...
      anon6:
        ...
        if ($non_null($phys_ptr_cast(L#curr, ^d_node)))
        {
          anon3:
            ...
            assert $writable_prim($s, #wrTime$2^3.3, $dot($phys_ptr_cast(L#tmp, ^d_node), d_node.next));
            // *(tmp->next) := rev; 
            ...
            if ($non_null($phys_ptr_cast(L#rev, ^d_node)))
            {
              anon1:
                ...
                assert $writable_prim($s, #wrTime$2^3.3, $dot($phys_ptr_cast(L#rev, ^d_node), d_node.prev));
                // *(rev->prev) := tmp; 
                call $write_int(d_node.prev, $phys_ptr_cast(L#rev, ^d_node), $ptr_to_int($phys_ptr_cast(L#tmp, ^d_node)));
                assume $full_stop_ext(#tok$2^23.7, $s);
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cgrass?5Cdl?5Cdl_reverse.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cgrass?5Cdl?5Cdl_reverse.c);
