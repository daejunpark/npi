...
const unique ^$#dl_remove.c..36263#3: $ctype;

axiom $def_fnptr_type(^$#dl_remove.c..36263#3);

type $#dl_remove.c..36263#3;
...
implementation dl_remove(P#lst: $ptr) returns ($result: $ptr)
{
  ...
  anon16:
    ...
    assume $non_null($phys_ptr_cast(P#lst, ^d_node)) ==> $mutable($s, $phys_ptr_cast(P#lst, ^d_node)) && $top_writable($s, #wrTime$2^5.3, $phys_ptr_cast(P#lst, ^d_node));
    ...
    else
    {
          ...
          anon7:
            ...
            if (ite#1)
            {
              anon4:
                ...
                // nondet := havoc(); 
                call L#nondet := \havoc();
                assume $full_stop_ext(#tok$2^26.16, $s);
      ...
      #break_3:
        ...
        assume $non_null($phys_ptr_cast(L#tmp, ^d_node)) ==> $mutable($s, $phys_ptr_cast(L#tmp, ^d_node)) && $top_writable($s, #wrTime$2^5.3, $phys_ptr_cast(L#tmp, ^d_node));
        ...
        if ($non_null($phys_ptr_cast(L#tmp, ^d_node)))
        {
          anon10:
            ...
            assert $writable_prim($s, #wrTime$2^5.3, $dot($phys_ptr_cast(L#curr, ^d_node), d_node.next));
            // *(curr->next) := tmp_next; 
            call $write_int(d_node.next, $phys_ptr_cast(L#curr, ^d_node), $ptr_to_int($phys_ptr_cast(L#tmp_next, ^d_node)));
            assume $full_stop_ext(#tok$2^36.7, $s);
            ...
            if ($non_null($phys_ptr_cast(L#tmp_next, ^d_node)))
            {
              anon8:
                ...
                assert $writable_prim($s, #wrTime$2^5.3, $dot($phys_ptr_cast(L#tmp_next, ^d_node), d_node.prev));
                // *(tmp_next->prev) := curr; 
                call $write_int(d_node.prev, $phys_ptr_cast(L#tmp_next, ^d_node), $ptr_to_int($phys_ptr_cast(L#curr, ^d_node)));
                assume $full_stop_ext(#tok$2^38.9, $s);
          ...
          anon11:
            ...
            assert $top_writable($s, #wrTime$2^5.3, $phys_ptr_cast(L#tmp, ^d_node));
            // assert @writes_check(@_vcc_extent(@state, tmp)); 
            assert (forall #writes$2^40.7: $ptr :: { $dont_instantiate(#writes$2^40.7) } $set_in(#writes$2^40.7, $extent($s, $phys_ptr_cast(L#tmp, ^d_node))) ==> $top_writable($s, #wrTime$2^5.3, #writes$2^40.7));
            // stmt _vcc_free(tmp); 
            call $free($phys_ptr_cast(L#tmp, ^d_node));
            assume $full_stop_ext(#tok$2^40.7, $s);
...
const unique #tok$2^40.7: $token;

const unique #tok$2^38.9: $token;

const unique #tok$2^36.7: $token;

const unique #tok$2^26.16: $token;
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cgrass?5Cdl?5Cdl_remove.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cgrass?5Cdl?5Cdl_remove.c);
