...
const unique ^$#dl_insert.c..36263#3: $ctype;

axiom $def_fnptr_type(^$#dl_insert.c..36263#3);

type $#dl_insert.c..36263#3;
  ...
  anon13:
    ...
    else
    {
      ...
      #break_3:
        ...
        assert $writable_prim($s, #wrTime$2^5.3, $dot($phys_ptr_cast(P#elt, ^d_node), d_node.next));
        // *(elt->next) := curr_next; 
        ...
        if ($non_null($phys_ptr_cast(L#curr_next, ^d_node)))
        {
          anon8:
            ...
            assert $writable_prim($s, #wrTime$2^5.3, $dot($phys_ptr_cast(L#curr_next, ^d_node), d_node.prev));
            // *(curr_next->prev) := elt; 
            call $write_int(d_node.prev, $phys_ptr_cast(L#curr_next, ^d_node), $ptr_to_int($phys_ptr_cast(P#elt, ^d_node)));
            assume $full_stop_ext(#tok$2^35.7, $s);
      ...
      anon12:
        ...
        assume $full_stop_ext(#tok$2^37.5, $s);
        ...
        assert $writable_prim($s, #wrTime$2^5.3, $dot($phys_ptr_cast(P#elt, ^d_node), d_node.prev));
        // *(elt->prev) := curr; 
        call $write_int(d_node.prev, $phys_ptr_cast(P#elt, ^d_node), $ptr_to_int($phys_ptr_cast(L#curr, ^d_node)));
        assume $full_stop_ext(#tok$2^38.5, $s);
...
const unique #tok$2^38.5: $token;

const unique #tok$2^37.5: $token;

const unique #tok$2^35.7: $token;
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cgrass?5Cdl?5Cdl_insert.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cgrass?5Cdl?5Cdl_insert.c);
