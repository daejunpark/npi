...
implementation sls_reverse(P#lst: $ptr) returns ($result: $ptr)
{
  ...
  anon4:
    ...
    while (true)
      ...
      invariant F#rsrtl($s, $phys_ptr_cast(L#rev, ^s_node));
      invariant $oset_disjoint(F#rsrtl_reach($s, $phys_ptr_cast(L#rev, ^s_node)), F#srtl_reach($s, $phys_ptr_cast(L#curr, ^s_node)));
      invariant $non_null($phys_ptr_cast(L#rev, ^s_node)) && $non_null($phys_ptr_cast(L#curr, ^s_node)) ==> F#sll_max_key($s, $phys_ptr_cast(L#rev, ^s_node)) <= $rd_inv($s, s_node.key, $phys_ptr_cast(L#curr, ^s_node));
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cgrass?5Csls?5Csls_reverse.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cgrass?5Csls?5Csls_reverse.c);
