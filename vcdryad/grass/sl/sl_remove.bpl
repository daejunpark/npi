...
const unique ^$#sl_remove.c..36263#3: $ctype;

axiom $def_fnptr_type(^$#sl_remove.c..36263#3);

type $#sl_remove.c..36263#3;
  ...
  anon13:
    ...
    else
    {
          ...
          anon7:
            ...
            if (ite#1)
            {
              anon4:
                ...
                // nondet := havoc(); 
                call L#nondet := \havoc();
                assume $full_stop_ext(#tok$2^27.16, $s);
      ...
      #break_3:
        ...
        assume $non_null($phys_ptr_cast(L#tmp, ^s_node)) ==> $mutable($s, $phys_ptr_cast(L#tmp, ^s_node)) && $top_writable($s, #wrTime$2^6.3, $phys_ptr_cast(L#tmp, ^s_node));
        ...
        if ($non_null($phys_ptr_cast(L#tmp, ^s_node)))
        {
          anon8:
            ...
            assert $top_writable($s, #wrTime$2^6.3, $phys_ptr_cast(L#tmp, ^s_node));
            // assert @writes_check(@_vcc_extent(@state, tmp)); 
            assert (forall #writes$2^38.7: $ptr :: { $dont_instantiate(#writes$2^38.7) } $set_in(#writes$2^38.7, $extent($s, $phys_ptr_cast(L#tmp, ^s_node))) ==> $top_writable($s, #wrTime$2^6.3, #writes$2^38.7));
            // stmt _vcc_free(tmp); 
...
const unique #tok$2^27.16: $token;
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cgrass?5Csl?5Csl_remove.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cgrass?5Csl?5Csl_remove.c);
