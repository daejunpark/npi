...
//             && sll_list_len_next(hd) == (sll_lseg_len_next(hd, tl) + sll_list_len_next(tl)) 
  ...
  _(ensures (hd != NULL && tl != NULL && sll(tl->next) && sll_lseg(hd, tl) 
...
_(logic \bool same_sll_all_un(struct s_node * hd, struct s_node * tl, \state enter, \state exit) = 
...
//_(logic _(dryad "cond:mkeys") 
//  \bool cond_same_sll_mkeys(struct s_node * x, struct s_node * y, \state enter, \state exit) =
//	  (! \oset_in(x, \at(enter, sll_reach(y)))) ==> \at(enter, sll_mkeys(y)) == \at(exit, sll_mkeys(y)) ;)
...
//  	&& cond_same_sll_mkeys(x, y, enter, exit) 
...
_(logic _(dryad "cond:lseg_keys")
  \bool cond_same_sll_lseg_keys(struct s_node * x, struct s_node * hd, struct s_node * tl, 
    ...
    ((! \oset_in(x, \at(enter, sll_lseg_reach(hd, tl)))) ==>
      \at(enter, sll_lseg_keys(hd, tl)) == \at(exit, sll_lseg_keys(hd, tl))) ;)
/*_(logic _(dryad "cond:lseg_mkeys")
  \bool cond_same_sll_lseg_mkeys(struct s_node * x, struct s_node * hd, struct s_node * tl, 
                            \state enter, \state exit) =
    ((! \oset_in(x, \at(enter, sll_lseg_reach(hd, tl)))) ==>
      \at(enter, sll_lseg_mkeys(hd, tl)) == \at(exit, sll_lseg_mkeys(hd, tl))) ;) */
...
_(logic 
  \bool cond_same_sll_all_bin(struct s_node * x, struct s_node * hd, struct s_node * tl, \state enter, \state exit) =
  	...
  	//&& cond_same_sll_lseg_mkeys(x, hd, tl, enter, exit)
