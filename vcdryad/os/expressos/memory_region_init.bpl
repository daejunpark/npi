...
procedure memory_region_init(P#file_: $ptr, P#file_offset_: int, P#file_size_: int, P#start_address_: int, P#size_: int) returns ($result: $ptr);
  ...
  ensures $is_null($rd_phys_ptr($s, mem_reg.prev, $phys_ptr_cast($result, ^mem_reg), ^mem_reg));
  ensures $is_null($rd_phys_ptr($s, mem_reg.next, $phys_ptr_cast($result, ^mem_reg), ^mem_reg));
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cos?5Cexpressos?5Cmemory_region_init.c: $token;

axiom $file_name_is(3, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cos?5Cexpressos?5Cmemory_region_init.c);
