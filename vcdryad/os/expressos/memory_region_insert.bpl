...
implementation memory_region_insert(P#head: $ptr, P#node: $ptr) returns ($result: $ptr)
{
  ...
  anon12:
    ...
    assume true;
    while (true)
      invariant F#dll($s, $phys_ptr_cast(L#curr, ^mem_reg));
    {
      anon11:
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cos?5Cexpressos?5Cmemory_region_insert.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cos?5Cexpressos?5Cmemory_region_insert.c);
