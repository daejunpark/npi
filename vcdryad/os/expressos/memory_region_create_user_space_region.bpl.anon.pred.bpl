...
implementation memory_region_create_user_space_region() returns ($result: $ptr)
{
  ...
  anon9:
    ...
    assume $full_stop_ext(#tok$3^37.17, $s);
    // _math \state _dryad_S1; 
    // _dryad_S1 := @_vcc_current_state(@state); 
    SL#_dryad_S1 := $current_state($s);
    ...
    assume $full_stop_ext(#tok$3^41.3, $s);
    // _math \state _dryad_S5; 
    // _dryad_S5 := @_vcc_current_state(@state); 
    SL#_dryad_S5 := $current_state($s);
    ...
    assume $full_stop_ext(#tok$3^42.3, $s);
    // _math \state _dryad_S7; 
    // _dryad_S7 := @_vcc_current_state(@state); 
    SL#_dryad_S7 := $current_state($s);
...
const unique #tok$3^42.3: $token;

const unique #tok$3^41.3: $token;
...
const unique #tok$3^37.17: $token;
...
const unique #file^Z?3A?5CInvariantSynthesis?5CVCDryad?5Cvcc?5CHost?5Cbin?5CTests?5Cmem?2Dregion?5Cmemory_region_create_user_space_region.c: $token;

axiom $file_name_is(3, #file^Z?3A?5CInvariantSynthesis?5CVCDryad?5Cvcc?5CHost?5Cbin?5CTests?5Cmem?2Dregion?5Cmemory_region_create_user_space_region.c);
