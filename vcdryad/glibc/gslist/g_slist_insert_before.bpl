...
implementation g_slist_insert_before(P#slist: $ptr, P#sibling: $ptr, P#data: int) returns ($result: $ptr)
{
  ...
  anon9:
    ...
    assume true;
    while (true)
      invariant F#sll($s, $phys_ptr_cast(L#node, ^s_node));
      invariant F#sll($s, $phys_ptr_cast(L#last, ^s_node));
      invariant F#sll_lseg($s, $phys_ptr_cast(local.slist, ^s_node), $phys_ptr_cast(L#node, ^s_node));
      invariant $oset_disjoint(F#sll_lseg_reach($s, $phys_ptr_cast(local.slist, ^s_node), $phys_ptr_cast(L#node, ^s_node)), F#sll_reach($s, $phys_ptr_cast(L#node, ^s_node)));
      invariant F#sll_lseg($s, $phys_ptr_cast(local.slist, ^s_node), $phys_ptr_cast(L#last, ^s_node));
      invariant $oset_disjoint(F#sll_lseg_reach($s, $phys_ptr_cast(local.slist, ^s_node), $phys_ptr_cast(L#last, ^s_node)), F#sll_reach($s, $phys_ptr_cast(L#last, ^s_node)));
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_insert_before.c: $token;

axiom $file_name_is(3, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_insert_before.c);
