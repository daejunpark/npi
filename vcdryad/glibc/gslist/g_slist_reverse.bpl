...
implementation g_slist_reverse(P#list: $ptr) returns ($result: $ptr)
{
  ...
  anon4:
    ...
    while (true)
      ...
      invariant $oset_disjoint(F#sll_reach($s, $phys_ptr_cast(local.list, ^s_node)), F#sll_reach($s, $phys_ptr_cast(L#prev, ^s_node)));
      ...
      invariant $intset_union(F#sll_keys($s, $phys_ptr_cast(local.list, ^s_node)), F#sll_keys($s, $phys_ptr_cast(L#prev, ^s_node))) == F#sll_keys(loopState#0, $phys_ptr_cast(P#list, ^s_node));
    {
      anon3:
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_reverse.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_reverse.c);
