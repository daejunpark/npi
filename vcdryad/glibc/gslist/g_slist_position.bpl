...
implementation g_slist_position(P#list: $ptr, P#link: $ptr) returns ($result: int)
{
  ...
  anon7:
    ...
    while (true)
      ...
      invariant !$oset_in($phys_ptr_cast(P#link, ^s_node), F#sll_lseg_reach($s, $phys_ptr_cast(P#list, ^s_node), $phys_ptr_cast(local.list, ^s_node)));
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_position.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_position.c);
