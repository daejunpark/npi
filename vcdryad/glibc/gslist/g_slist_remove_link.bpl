...
implementation g_slist_remove_link(P#list: $ptr, P#link: $ptr) returns ($result: $ptr)
{
  ...
  anon10:
    ...
    while (true)
      ...
      invariant F#sll_reach($s, $phys_ptr_cast(local.list, ^s_node)) == $oset_union(F#sll_lseg_reach($s, $phys_ptr_cast(local.list, ^s_node), $phys_ptr_cast(L#prev, ^s_node)), F#sll_reach($s, $phys_ptr_cast(L#prev, ^s_node)));
      invariant F#sll_reach($s, $phys_ptr_cast(local.list, ^s_node)) == $oset_union(F#sll_lseg_reach($s, $phys_ptr_cast(local.list, ^s_node), $phys_ptr_cast(L#tmp, ^s_node)), F#sll_reach($s, $phys_ptr_cast(L#tmp, ^s_node)));
      invariant F#sll_reach($s, $phys_ptr_cast(local.list, ^s_node)) == F#sll_reach(loopState#0, $phys_ptr_cast(P#list, ^s_node));
      invariant $oset_subset(F#sll_reach($s, $phys_ptr_cast(L#prev, ^s_node)), F#sll_reach($s, $phys_ptr_cast(local.list, ^s_node)));
      invariant $oset_subset(F#sll_reach($s, $phys_ptr_cast(L#tmp, ^s_node)), F#sll_reach($s, $phys_ptr_cast(local.list, ^s_node)));
      invariant !$oset_in($phys_ptr_cast(P#link, ^s_node), F#sll_lseg_reach($s, $phys_ptr_cast(local.list, ^s_node), $phys_ptr_cast(L#tmp, ^s_node)));
      invariant $is_null($phys_ptr_cast(L#prev, ^s_node)) ==> $phys_ptr_cast(L#tmp, ^s_node) == $phys_ptr_cast(local.list, ^s_node);
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_remove_link.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_remove_link.c);
