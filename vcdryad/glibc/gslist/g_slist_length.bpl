...
implementation g_slist_length(P#list: $ptr) returns ($result: int)
{
  ...
  anon4:
    ...
    while (true)
      ...
      invariant F#sll_list_len_next($s, $phys_ptr_cast(P#list, ^s_node)) - F#sll_list_len_next($s, $phys_ptr_cast(local.list, ^s_node)) == L#length;
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_length.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_length.c);
