...
implementation g_slist_index(P#list: $ptr, P#data: int) returns ($result: int, OP#ret_list: $ptr)
{
  ...
  anon7:
    ...
    while (true)
      ...
      invariant F#sll_list_len_next($s, $phys_ptr_cast(P#list, ^s_node)) - F#sll_list_len_next($s, $phys_ptr_cast(local.list, ^s_node)) == L#i;
      invariant !$intset_in(P#data, F#sll_lseg_keys($s, $phys_ptr_cast(P#list, ^s_node), $phys_ptr_cast(local.list, ^s_node)));
      invariant F#sll_lseg_len_next($s, $phys_ptr_cast(P#list, ^s_node), $phys_ptr_cast(local.list, ^s_node)) == L#i;
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_index.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_index.c);
