...
implementation g_slist_nth(P#list: $ptr, P#n: int) returns ($result: $ptr)
{
  ...
  anon4:
    ...
    while (true)
      ...
      invariant $non_null($phys_ptr_cast(local.list, ^s_node)) ==> P#n - local.n == F#sll_list_len_next($s, $phys_ptr_cast(P#list, ^s_node)) - F#sll_list_len_next($s, $phys_ptr_cast(local.list, ^s_node));
      invariant F#sll_list_len_next($s, $phys_ptr_cast(P#list, ^s_node)) == $unchk_add(^^nat, F#sll_lseg_len_next($s, $phys_ptr_cast(P#list, ^s_node), $phys_ptr_cast(local.list, ^s_node)), F#sll_list_len_next($s, $phys_ptr_cast(local.list, ^s_node)));
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_nth.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cgslist?5Cg_slist_nth.c);
