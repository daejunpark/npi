...
const unique ^$#g_list_index.c..36263#3: $ctype;

axiom $def_fnptr_type(^$#g_list_index.c..36263#3);

type $#g_list_index.c..36263#3;
...
implementation g_list_index(P#list: $ptr, P#data: int) returns ($result: int, OP#ret_list: $ptr)
{
  ...
  anon7:
    ...
    while (true)
      ...
      invariant !$intset_in(P#data, F#dll_lseg_keys($s, $phys_ptr_cast(P#list, ^d_node), $phys_ptr_cast(local.list, ^d_node)));
      ...
      invariant $non_null($phys_ptr_cast(local.list, ^d_node)) ==> $top_writable($s, #wrTime$2^5.3, $phys_ptr_cast(local.list, ^d_node));
    {
      anon6:
        ...
        assume $full_stop_ext(#tok$2^23.3, $s);
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cglist?5Cg_list_index.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cglist?5Cg_list_index.c);
