...
const unique ^$#g_list_last.c..36263#3: $ctype;

axiom $def_fnptr_type(^$#g_list_last.c..36263#3);

type $#g_list_last.c..36263#3;
    ...
    if ($non_null($phys_ptr_cast(local.list, ^d_node)))
    {
      anon4:
        ...
        while (true)
          ...
          invariant $oset_disjoint(F#dll_reach($s, $phys_ptr_cast(local.list, ^d_node)), F#dll_lseg_reach($s, $phys_ptr_cast(P#list, ^d_node), $phys_ptr_cast(local.list, ^d_node)));
          invariant F#dll_keys($s, $phys_ptr_cast(local.list, ^d_node)) == $intset_union(F#dll_keys($s, $rd_phys_ptr($s, d_node.next, $phys_ptr_cast(local.list, ^d_node), ^d_node)), $intset_singleton($rd_inv($s, d_node.key, $phys_ptr_cast(local.list, ^d_node))));
          invariant $non_null($phys_ptr_cast(local.list, ^d_node));
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cglist?5Cg_list_last.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cglist?5Cg_list_last.c);
