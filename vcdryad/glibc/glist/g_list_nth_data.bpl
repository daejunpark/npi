...
const unique ^$#g_list_nth_data.c..36263#3: $ctype;

axiom $def_fnptr_type(^$#g_list_nth_data.c..36263#3);

type $#g_list_nth_data.c..36263#3;
...
implementation g_list_nth_data(P#list: $ptr, P#n: int) returns ($result: int, OP#ret_list: $ptr)
{
  ...
  anon6:
    ...
    while (true)
      ...
      invariant F#dll($s, $phys_ptr_cast(OP#ret_list, ^d_node));
      invariant $phys_ptr_cast(OP#ret_list, ^d_node) == $phys_ptr_cast(local.list, ^d_node);
      ...
      invariant $non_null($phys_ptr_cast(local.list, ^d_node)) ==> F#dll_list_len_next($s, $phys_ptr_cast(P#list, ^d_node)) - F#dll_list_len_next($s, $phys_ptr_cast(local.list, ^d_node)) == P#n - local.n;
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cglist?5Cg_list_nth_data.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cglist?5Cg_list_nth_data.c);
