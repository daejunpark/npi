...
const unique ^$#g_list_free.c..36263#3: $ctype;

axiom $def_fnptr_type(^$#g_list_free.c..36263#3);

type $#g_list_free.c..36263#3;
...
implementation g_list_free(P#list: $ptr) returns (OP#ALL_REACH: $oset)
{
  ...
  anon4:
    ...
    assume $full_stop_ext(#tok$2^12.23, $s);
    // x := list; 
    ...
    assume true;
    while (true)
      invariant F#dll($s, $phys_ptr_cast(L#x, ^d_node));
      invariant $non_null($phys_ptr_cast(L#x, ^d_node)) ==> $mutable($s, $phys_ptr_cast(L#x, ^d_node));
      invariant $non_null($phys_ptr_cast(L#x, ^d_node)) ==> $top_writable($s, #wrTime$2^5.3, $phys_ptr_cast(L#x, ^d_node));
      ...
      anon3:
        ...
        if ($non_null($phys_ptr_cast(L#x, ^d_node)))
        {
          anon1:
            ...
            assert $top_writable($s, #wrTime$2^5.3, $phys_ptr_cast(L#x, ^d_node));
            // assert @writes_check(@_vcc_extent(@state, x)); 
            assert (forall #writes$2^23.5: $ptr :: { $dont_instantiate(#writes$2^23.5) } $set_in(#writes$2^23.5, $extent($s, $phys_ptr_cast(L#x, ^d_node))) ==> $top_writable($s, #wrTime$2^5.3, #writes$2^23.5));
            // stmt _vcc_free(x); 
            ...
            assume $non_null($phys_ptr_cast(L#x, ^d_node)) ==> $mutable($s, $phys_ptr_cast(L#x, ^d_node)) && $top_writable($s, #wrTime$2^5.3, $phys_ptr_cast(L#x, ^d_node));
        }
        else
        {
          anon2:
...
const unique #tok$2^12.23: $token;
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cglist?5Cg_list_free.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cglist?5Cg_list_free.c);
