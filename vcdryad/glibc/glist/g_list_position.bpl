...
const unique ^$#g_list_position.c..36263#3: $ctype;

axiom $def_fnptr_type(^$#g_list_position.c..36263#3);

type $#g_list_position.c..36263#3;
...
implementation g_list_position(P#list: $ptr, P#link: $ptr) returns ($result: int)
{
  ...
  anon7:
    ...
    assume $non_null($phys_ptr_cast(local.list, ^d_node)) ==> $mutable($s, $phys_ptr_cast(local.list, ^d_node)) && $top_writable($s, #wrTime$2^5.3, $phys_ptr_cast(local.list, ^d_node));
    // var int32_t i
    // i := 0; 
    L#i := 0;
    loopState#0 := $s;
    ...
    while (true)
      ...
      invariant !$oset_in($phys_ptr_cast(P#link, ^d_node), F#dll_lseg_reach($s, $phys_ptr_cast(P#list, ^d_node), $phys_ptr_cast(local.list, ^d_node)));
      invariant F#dll_list_len_next($s, $phys_ptr_cast(P#list, ^d_node)) - F#dll_list_len_next($s, $phys_ptr_cast(local.list, ^d_node)) == L#i;
      invariant F#dll_lseg_len_next($s, $phys_ptr_cast(P#list, ^d_node), $phys_ptr_cast(local.list, ^d_node)) == L#i;
      invariant L#i < 2147483647;
      invariant L#i >= 0;
      invariant $non_null($phys_ptr_cast(local.list, ^d_node)) ==> $mutable($s, $phys_ptr_cast(local.list, ^d_node));
      invariant $non_null($phys_ptr_cast(local.list, ^d_node)) ==> $top_writable($s, #wrTime$2^5.3, $phys_ptr_cast(local.list, ^d_node));
    {
      anon6:
        assume $writes_nothing(old($s), $s);
        assume $timestamp_post(loopState#0, $s);
        assume $full_stop_ext(#tok$2^21.3, $s);
...
const unique #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cglist?5Cg_list_position.c: $token;

axiom $file_name_is(2, #file^Z?3A?5CInvariantSynthesis?5Csubmit?5Cglibc?5Cglist?5Cg_list_position.c);
