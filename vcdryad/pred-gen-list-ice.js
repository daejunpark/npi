    ...
    for (i = 0; i < patterns_heap.length; i++) {
        ...
        if (numptrs(patterns_heap[i]) <= ptrs.length) {
            ...
            for (j = 0; j < curr_ptrs_permutation.length; j++) {
                result_boogie.push(ts(patterns_heap[i], curr_ptrs_permutation[j], []));
            }
        }
    }
        ...
        if (numptrs(patterns_data[i]) <= ptrs.length && numints(patterns_data[i]) <= ints.length) {
            ...
            for (j = 0; j < curr_ptrs_permutation.length; j++) {
                for (k = 0; k < curr_ints_permutation.length; k++) {
                    result_boogie.push(ts(patterns_data[i], curr_ptrs_permutation[j], curr_ints_permutation[k]));
                }
            }
        }
    }
    generate_ice(result_boogie);
}

function generate_ice(result){
    for (i = 0; i < result.length; i++) {
       console.log('p' + pad(i,digits) + ' := (' + result[i] + ');');
    }

    console.log('INV');
    for (i = 0; i < result.length; i++) {
        console.log('invariant (p' + pad(i,digits) + ' == (' + result[i] + '));');
    }

    console.log('INV');
    for (i = 0; i < result.length; i++) {
        console.log('var p' + pad(i,digits) + ' : bool;');
    }

    console.log('INV');
    console.log('invariant my_inv (');
    for (i = 0; i < result.length; i++) {
        console.log('p' + pad(i,digits) + ',');
    }
    console.log('true');
    console.log(');');

    console.log('INV');
    console.log('function {:existential true} my_inv (');
    for (i = 0; i < result.length; i++) {
        console.log('b' + pad(i,digits) + ' : bool,');
    }
    console.log('b' + pad(i,digits) + ' : bool');
    console.log(') : bool;');
}
